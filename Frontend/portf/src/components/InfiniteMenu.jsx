...
uniform float uAtlasSize; // Changed to float to simplify divisions
...
void main() {
    ...
    // Draw as a circle by discarding pixels outside radius 0.5
...
class InfiniteGridMenu {
    ...
    Promise.all(
      this.items.map(
        item =>
          new Promise(resolve => {
            const img = new Image();
            img.crossOrigin = 'anonymous'; // Required for Unsplash and WebGL texture mapping
            img.onload = () => resolve(img);
            img.onerror = () => {
              console.error("Failed to load image", item.image);
              // Resolve with empty canvas to avoid hanging Promise.all
  ...
  #render() {
    ...
    gl.uniform1f(this.discLocations.uAtlasSize, this.atlasSize); // Updated to float
...
export default function InfiniteMenu({ items = [], scale = 2.3, onItemChange }) {
